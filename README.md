...
After the header, you can have one to five lines for the dialogue. At any point in the dialogue, you can have a square bracket argument to change various parameters for the text. Additionally, putting a string of text in curly brackets makes the text appear instantly instead of one character at a time.
...
id - Determines which instance of the dialogue box is used. It allows numeric values 0-9.
...
Canceling dialogue:  
The function chatbox:visible/off/<id> will wipe the specific dialogue box and stop the current dialogue from printing. It does not, however, cancel scheduled functions in the dialogue, those will have to be canceled manually.
